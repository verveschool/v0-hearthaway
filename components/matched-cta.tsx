import Link from 'next/link'

type MatchedCTAProps = {
  title: React.ReactNode
  description?: React.ReactNode
  buttonText?: string
  buttonHref?: string
  variant?: 'full' | 'compact'
  className?: string
}

export default function MatchedCTA({
  title,
  description,
  buttonText = 'Get Matched',
  buttonHref = '/get-matched',
  variant = 'full',
  className = '',
}: MatchedCTAProps) {
  if (variant === 'compact') {
    return (
      <div className={`bg-[#F7F6F3] rounded-2xl p-6 border border-[#E8E6E1] flex flex-col sm:flex-row items-start justify-between gap-4 ${className}`}>
        <div>
          <h3 className="font-heading font-bold text-lg text-[#1A1A1A] mb-1">{title}</h3>
          {description && <p className="text-[#6B6860] text-sm">{description}</p>}
        </div>

        <div className="w-full flex-shrink-0 sm:w-auto">
          <Link
            href={buttonHref}
            className="block w-full px-6 py-2.5 bg-[#00319D] text-center text-white font-bold rounded-xl hover:bg-[#00319D] transition-colors text-sm sm:w-auto"
          >
            {buttonText}
          </Link>
        </div>
      </div>
    )
  }

  return (
    <div className={`bg-[#00319D] rounded-2xl p-8 lg:p-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 ${className}`}>
      <div className="max-w-xl">
        <h3 className="font-heading font-bold text-2xl lg:text-3xl text-white mb-2">{title}</h3>
        {description && <p className="text-white/70 text-base">{description}</p>}
      </div>

      <Link
        href={buttonHref}
        className="w-full flex-shrink-0 px-6 py-3 bg-[#FCC20A] text-center text-[#333333] font-bold text-sm rounded-xl hover:bg-[#FCC20A] transition-colors shadow-lg sm:w-auto lg:px-8 lg:py-4 lg:text-base"
      >
        {buttonText}
      </Link>
    </div>
  )
}
