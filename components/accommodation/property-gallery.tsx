      ...
      rooms
        ...
        .filter(
          (image): image is string =>
            Boolean(image) && !propertyImages.includes(image as string),
        ),
    [rooms, propertyImages],
  );
  const combinedImages = useMemo(
    () => [...propertyImages, ...roomOnlyImages],
    [propertyImages, roomOnlyImages],
  );
  const roomImageIndex = new Map(
    rooms
      .filter((room) => room.image)
      .map((room) => [room.image as string, room.name]),
  );
  const activeRoom = activeRoomName
    ? rooms.find((room) => room.name === activeRoomName)
    : undefined;
  const initialIndex = activeRoom?.image
    ? Math.max(combinedImages.indexOf(activeRoom.image), 0)
    : 0;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft")
        setLightboxIndex((index) =>
          index === null
            ? 0
            : (index - 1 + combinedImages.length) % combinedImages.length,
        );
      if (event.key === "ArrowRight")
        setLightboxIndex((index) =>
          index === null ? 0 : (index + 1) % combinedImages.length,
        );
    };
    ...
    <>
      <div className="relative overflow-hidden rounded-2xl bg-[#00319D] shadow-xl">
        <button
          ...
          className="group relative block h-[360px] w-full text-left focus:outline-none focus-visible:ring-4 focus-visible:ring-[#FCC20A] sm:h-[500px]"
          aria-label={`Open ${name} photo viewer`}
        >
          <img
            src={activeImage}
            alt={activeLabel ? `${activeLabel} at ${name}` : name}
            className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.02]"
          />
          <div
            className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent pt-16"
            aria-hidden="true"
          />
          <span className="absolute right-5 top-5 rounded-full bg-black/60 px-4 py-2 text-xs font-bold text-white">
            View photos
          </span>
          {activeLabel ? (
            <span className="absolute left-5 top-5 rounded-full bg-[#FCC20A] px-4 py-2 text-xs font-bold text-[#1A1A1A]">
              {activeLabel} room photo
            </span>
          ) : null}
        </button>
        <div className="pointer-events-none absolute bottom-7 left-7 right-7 text-white sm:bottom-9 sm:left-9 sm:right-9">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#FCC20A]">
            <Link
              href={cityHref}
        ...
        {combinedImages.slice(1, 9).map((image, index) => (
          <button
            ...
            onClick={() => {
              ...
              setSelectedRoom(roomImageIndex.get(image) ?? null);
            }}
            className={`group overflow-hidden rounded-xl border bg-white text-left focus:outline-none focus-visible:ring-4 focus-visible:ring-[#FCC20A] ${activeIndex === index + 1 ? "border-[#00319D]" : "border-[#E8E6E1]"}`}
            aria-label={`Show ${roomImageIndex.get(image) ? `${roomImageIndex.get(image)} room photo` : `photo ${index + 2}`}`}
          >
            <img
              src={image}
              alt={
                roomImageIndex.get(image)
                  ? `${roomImageIndex.get(image)} at ${name}`
                  : `${name} photo ${index + 2}`
              }
      ...
      <div className="mt-6 rounded-2xl border border-[#E8E6E1] bg-white p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm leading-relaxed text-[#6B6860]">
          Every room type offered at this property, as listed by the source.
          Select a room type to view its own photo above; a room type stays
          listed here.
        </p>
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <p
                    className={`mt-2 font-heading text-xl font-extrabold ${room.price.priceOnEnquiry ? "text-[#6B6860]" : "text-[#00319D]"}`}
                  >
                    {formatRoomPrice(room.price)}
                  </p>
                  <p className="mt-1 text-[11px] font-bold uppercase tracking-wide text-[#6B6860]">
                    Tentative price &mdash; subject to change, dates and
                    contract terms
                  </p>
                  {room.price.conditions ? (
                    <p className="mt-1 text-xs leading-relaxed text-[#6B6860]">
                      {room.price.conditions}
                    </p>
                  ) : null}
                  {room.tenancy ? (
                    <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-[#6B6860]">
                      {room.tenancy}
                    </p>
                  ) : null}
        ...
        >
          <div className="flex h-full flex-col">
            <header className="flex shrink-0 items-center justify-between border-b border-white/10 px-5 py-4 text-white sm:px-8">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#FCC20A]">
                  {roomImageIndex.get(combinedImages[lightboxIndex])
                    ? `${roomImageIndex.get(combinedImages[lightboxIndex])} room photo`
                    : "Property gallery"}
                </p>
              ...
              <div className="flex items-center gap-4">
                <span className="text-sm font-medium text-white/60">
                  {lightboxIndex + 1} / {combinedImages.length}
                </span>
                <button
                  type="button"
                  onClick={() => setLightboxIndex(null)}
                  className="rounded-full border border-white/15 bg-white/10 p-2.5 text-white transition hover:bg-white/20 focus:outline-none focus-visible:ring-4 focus-visible:ring-[#FCC20A]"
            ...
            <div className="relative flex min-h-0 flex-1 items-center justify-center px-12 py-6 sm:px-24 sm:py-8">
              <button
                ...
                className="absolute left-3 z-10 rounded-full border border-white/15 bg-white/10 p-3 text-white backdrop-blur transition hover:bg-white/20 focus:outline-none focus-visible:ring-4 focus-visible:ring-[#FCC20A] sm:left-8"
              ...
              <div className="h-[min(72vh,760px)] w-[min(88vw,1280px)] px-2 sm:px-4">
                <img
                  src={combinedImages[lightboxIndex]}
                  alt={
                    roomImageIndex.get(combinedImages[lightboxIndex])
                      ? `${roomImageIndex.get(combinedImages[lightboxIndex])} at ${name}`
                      : `${name} photo ${lightboxIndex + 1}`
                  }
              ...
              <button
                type="button"
                onClick={() =>
                  setLightboxIndex((lightboxIndex + 1) % combinedImages.length)
                }
                className="absolute right-3 z-10 rounded-full border border-white/15 bg-white/10 p-3 text-white backdrop-blur transition hover:bg-white/20 focus:outline-none focus-visible:ring-4 focus-visible:ring-[#FCC20A] sm:right-8"
              ...
              >
                {combinedImages.map((image, index) => (
                  <button
                    ...
                    className={`h-14 w-20 shrink-0 overflow-hidden rounded-md border-2 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FCC20A] ${index === lightboxIndex ? "border-[#FCC20A]" : "border-transparent opacity-60 hover:opacity-100"}`}
