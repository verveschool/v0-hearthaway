  ...
  return <>
    <Navigation /><main className="bg-[#F7F6F3]">
      <section className="px-6 py-8 lg:px-8 lg:py-12"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.4fr_0.6fr]">
        <div>
          <div className="relative"><ImageLightbox images={gallery} name={property.name} /><div className="pointer-events-none absolute bottom-7 left-7 right-7 text-white sm:bottom-9 sm:left-9 sm:right-9"><p className="text-sm font-bold uppercase tracking-[0.18em] text-[#FCC20A]"><Link href={`/cities/${property.city.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="pointer-events-auto hover:underline">{property.city}</Link>, {property.country}</p><h1 className="mt-2 font-heading text-4xl font-extrabold leading-tight sm:text-5xl">{property.name}</h1></div></div>
