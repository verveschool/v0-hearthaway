...
import { formatAccommodationPricePeriod, formatRoomPrice } from './formatters'
...
function AccommodationContent() {
  ...
  // Each card below is one room-type listing, not one property, so a property with
  // three sourced room types produces three distinct, independently filterable cards.
  const filteredListings = useMemo(() => accommodationListings.filter((listing) => {
    const { property, room } = listing
    const normalizedQuery = query.trim().toLowerCase()
    const matchesQuery = !normalizedQuery || [property.name, property.city, room.name, ...property.universities, ...property.amenities, ...(property.goodFor ?? [])].some((value) => value.toLowerCase().includes(normalizedQuery))
    const matchesCountry = country === 'All countries' || property.country === country
    const matchesCity = city === 'All cities' || property.city === city
    const matchesRoom = roomType === 'Any room type' || room.name === roomType
    const matchesCategory = category === 'Any category' || (property.categories ?? []).includes(category)
    const matchesBudget = !selectedBudget || (
      room.price.currency === selectedBudget.currency
      && room.price.period === selectedBudget.pricePeriod
      && !room.price.priceOnEnquiry
      && (room.price.value ?? room.price.minValue) !== undefined
      && (room.price.value ?? room.price.minValue ?? 0) <= selectedBudget.upperBound
    )
  ...
  return <>
    ...
    <main>
      ...
      <section className="bg-[#F7F6F3] px-6 py-14 lg:px-8 lg:py-20"><div className="mx-auto max-w-7xl"><div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="mb-3 inline-flex items-center gap-2"><SlidersHorizontal className="h-4 w-4 text-[#00319D]" /><span className="text-sm font-bold uppercase tracking-wider text-[#00319D]">Explore</span></div><h2 className="font-heading text-3xl font-extrabold tracking-tight text-[#1A1A1A] sm:text-4xl">Places to start with</h2><p className="mt-2 text-sm text-[#6B6860]">{filteredListings.length} sourced room-type listings with pricing; room-photo checks are shown on each card.</p></div><Link href="/get-matched" className="inline-flex items-center gap-2 text-sm font-bold text-[#00319D]">Prefer a recommendation? Get matched <ArrowRight className="h-4 w-4" /></Link></div>{filteredListings.length === 0 ? <div className="rounded-2xl border border-[#E8E6E1] bg-white p-12 text-center"><h3 className="font-heading text-xl font-extrabold">No properties match those filters.</h3><p className="mt-2 text-sm text-[#6B6860]">Try a different city, category, room type or budget.</p></div> : <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{filteredListings.map((listing) => { const { property, room } = listing; return <Link key={listing.slug} href={`/accommodation/${listing.slug}`} className="group overflow-hidden rounded-2xl border border-[#E8E6E1] bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"><div className="relative h-56 overflow-hidden bg-[#00319D]"><img src={room.image ?? property.image} alt={`${room.name} at ${property.name}`} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" /><div className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 text-xs font-bold text-[#00319D]"><CheckCircle2 className="h-3.5 w-3.5" />{listing.hasVerifiedRoomPhoto ? 'Specific room photo verified' : 'Room photo to confirm'}</div><div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-4 text-white"><div><p className="text-xs font-semibold uppercase tracking-wider text-white/70">{property.city}</p><h3 className="font-heading text-2xl font-extrabold leading-tight">{property.name}</h3><p className="mt-0.5 text-sm font-bold text-[#FCC20A]">{room.name}</p></div><div className="shrink-0 text-right"><p className="text-xs text-white/70">from</p><p className="text-lg font-extrabold">{formatRoomPrice(room.price)}</p></div></div></div><div className="p-5"><div className="flex items-start gap-2 text-sm text-[#6B6860]"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#00319D]" /><span>{property.address}</span></div><div className="mt-4 flex flex-wrap gap-2">{property.roomTypes.slice(0, 3).map((type) => <span key={type} className="rounded-full bg-[#F7F6F3] px-3 py-1 text-xs font-semibold text-[#1A1A1A]">{type}</span>)}</div><div className="mt-5 border-t border-[#E8E6E1] pt-4"><p className="text-xs font-semibold uppercase tracking-wider text-[#6B6860]">Nearby</p><p className="mt-1 text-sm font-semibold text-[#1A1A1A]">{property.distance}</p></div></div></Link>})}</div>}</div></section>
    </main>
    <Footer />
  </>
}
