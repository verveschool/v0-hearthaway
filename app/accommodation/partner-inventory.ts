...
// Strips accents before slugifying (e.g. "Neukölln" -> "neukolln") so names with
// diacritics produce the same slug shape everything else keys off of — without
...
export const additionalAccommodationProperties: CatalogProperty[] = [
  makeProperty({
    ...
    sourceUrl: 'https://crm-students.com/hatfield/luna',
    priceFrom: 0,
    currency: 'GBP',
    pricePeriod: 'week',
    roomTypes: ['Studio', '2 Bed Apartment'],
    ...
    amenities: ['All utilities included', 'High-speed Wi-Fi', 'Free on-site gym', 'Cinema room', 'Games room', 'Karaoke room', 'Private dining room', 'Landscaped garden and BBQ area', 'Laundry facilities', 'Study spaces', 'Bike storage', 'CCTV', 'On-site maintenance', 'Parcel service', 'Keycard access'],
    highlights: ['Opposite De Havilland campus', 'Modern studios and two-bed apartments', 'Free on-site sports centre', 'Extensive resident social spaces'],
    roomFeatures: ['Private en-suite bathroom', 'Small double bed', 'Study area', 'Smart TV', 'Wardrobe', 'Private kitchen area in studios'],
    inclusions: ['All utility bills', 'High-speed Wi-Fi', 'Access to gym and communal spaces', 'On-site support'],
  ...
  makeProperty({
    ...
    name: 'iQ Fiveways House',
    city: 'Manchester',
    country: 'UK',
    address: '55 Chester Road, Manchester, M15 6JX',
    partnerSlug: 'iq-student-accommodation',
    sourceUrl: 'https://www.iqstudentaccommodation.com/manchester/fiveways-house',
    priceFrom: 0,
    currency: 'GBP',
    pricePeriod: 'week',
    roomTypes: ['Ensuite', 'Studio', 'Private Room'],
    categories: ['Student residence', 'Purpose-built', 'Furnished'],
    universities: ['University of Manchester', 'Manchester Metropolitan University'],
    distance: 'Close to the University of Manchester and Manchester Metropolitan University',
    image: '/images/acc-halls.png',
    gallery: ['/images/acc-halls.png', '/images/acc-studio.png', '/images/acc-kitchen.png', '/images/acc-shared.png'],
    amenities: ['Wi-Fi', 'Study spaces', 'Gym', 'Cinema room', 'Games room', 'Laundry', 'Bike storage', '24/7 support'],
    highlights: ['Popular student location', 'Private room and studio options', 'Social spaces for residents'],
    roomFeatures: ['Private en-suite bathroom', 'Study desk and chair', 'Bed and storage', 'Fitted kitchen or kitchenette', 'High-speed internet'],
    inclusions: ['Wi-Fi', 'Furnished room', 'Access to shared spaces', 'On-site support'],
    contractTerms: ['Availability and contract length vary by room type.', 'Confirm the current offer before booking.'],
    depositNote: 'Deposit and advance payment requirements vary by room type; confirm before booking.',
    goodFor: ['Students studying in central Manchester', 'Students who want shared social facilities', 'Students comparing ensuite and studio living'],
  }),
  ...
  ...neonWood.map(([name, city, sourceUrl]) => makeProperty({ slug: `neon-wood-${slugify(name)}`, name, city, country: 'Germany', address: city, priceFrom: 0, currency: 'EUR', pricePeriod: 'month', partnerSlug: 'neon-wood', sourceUrl, ...neonShared, distance: `Well connected to universities in ${city}`, contractTerms: ['6 or 12 months', 'Shorter stays may be available during promotions'], depositNote: 'Deposit applies; confirm the current amount for the selected apartment.' })),
  ...vitaStudent.map(([name, city]) => makeProperty({ slug: `vita-student-${slugify(name)}`, name, city, country: city === 'Barcelona' || city === 'Madrid' ? 'Spain' : 'UK', address: `${name}, ${city}`, partnerSlug: 'vita-student', pricePeriod: 'week', sourceUrl: `${sourceByPartner['vita-student']}${city.toLowerCase()}/`, ...vitaShared, distance: `Central student location in ${city}` })),
